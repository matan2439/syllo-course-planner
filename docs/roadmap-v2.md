...
**Confirmed 2026-07-01:** verified by grepping every `tracer.record()` call site in `api/` — none construct these three action types, and the frontend never filters/renders on them. Not a compatibility concern (no producer, no consumer) — safe to delete outright. Do not resurrect them as trace actions if validation/scoring observability is wanted later; that belongs in `BeamSearchMeta.depthRecords` (which already carries `CandidateRecord.rejectReason`), not as sibling values of the course-mutation enum. See `§8` roadmap item 7.
...
### 7.4 Wire `generateCandidates` into Production Flow — ✅ DONE (P2-A, commit d86252a, 2026-07-01)

Superseded by beam search (Section 2.2). Terminal beams replace `generateCandidates`. Dead code removed.
...
## Section 8 — Architecture Review Findings (2026-07-01)

Full review conducted against `docs/product-goal.md`'s generic-engine vision (7-section brief: annual dedup, trace API, rule-engine hardcoding audit, search layer, knowledge capability, single sources of truth, future generic platform). **No code was changed as part of this review.** Full reasoning is in that session's transcript; durable findings are also carried in `.remember/remember.md`. Summary:

**What's already right (don't undo it):** `ConstraintModel`/`CategoryReq`/`CourseProfile` are more generic than a "still course-centric" read would suggest — `knownSemesterIds`, `categories`, and `degreeRequiredHours` are all data-driven from `board_json`, not hardcoded. `SearchStrategy`/`BeamSearchStrategy` verifiably never import `ConstraintModel` (Design Invariant 1 holds). `detectGaps` is verifiably pure.
...
5. **`PlannerAgent.run()` is a fixed 5-step linear pipeline, not a resumable/adaptive loop** — `BeamSearchStrategy.explore()` has no pause/resume point, so knowledge-gap resolution can only ever happen pre-search, never mid-search. Needs a resumable-search seam before real `KnowledgeCapability` is wired in.
6. **The client JS shadow engine (`semester_board_viewer.html`, ~20 `*Local` functions) is the single largest "one canonical implementation" violation in the codebase** — it independently reimplements scoring, validation, repair, and degree-completion logic. This is Section 7.5 but its true scope is larger than that section implies; retiring it resolves most of the other "single source of truth" gaps at once.
...
**No roadmap item above has been approved for implementation yet — this is a review, not a plan.** Start the next session here:

1. Re-verify test suite state first (see `.remember/remember.md` "Test Suite State" — `npm test` had 4 pre-existing failures in `tests/ui/planner_user_intent_focus.test.js`, jsdom timeout, not investigated; `pytest` could not run in the sandbox at all due to environment/interpreter issues, unrelated to any code change). Confirm these are still the same before assuming either is fixed or worse.
2. Get explicit confirmation on which `§8` roadmap item to start with. **Recommended: item 1** (fix the live annual-hours double-counting bug, §4.3) — verified production correctness bug, small blast radius, and its fix (derive `root_course_id` at board-extraction time instead of hand-patching) sets the right precedent before more one-off per-course scripts get written.
3. If proceeding with item 1: write a failing test first (TDD) against the *real* `buildCourseProfiles`/`buildConstraintModel` path over `data/boards/mechanical_engineering_2027.json` — not a fixture that sets `root_course_id` by hand, which is exactly the kind of shortcut that hid this bug the first time (same pattern as the hard-avoid test blind spot documented in `.remember/`). Then derive `root_course_id` in the board extraction/enrichment step, lift `is_annual`/`spans_semesters` into `CourseProfile`, and tighten `board_audit.py` to check the same fields the planner actually reads.
