...
${preferences.max_weekly_hours != null ? `- מגבלת השעות השבועיות (${preferences.max_weekly_hours} ש״ש לסמסטר) היא אילוץ מחייב: חלק את הקורסים בין הסמסטרים כך שאף סמסטר לא יחרוג ממנה, גם אם המשמעות היא לפזר קורסי בחירה גמישים בין סמסטרים שונים. אם פיזור מוחלט בלתי אפשרי בגלל קורסים שמותרים רק בסמסטר מסוים, צמצם את החריגה למינימום האפשרי וציין זאת במפורש ב-warnings_he יחד עם הסיבה (למשל קורסי חובה קבועים שאי אפשר להזיז).` : ''}
