/**
 * W-browser — build the COMMITTED deterministic evidence fixture used by the
 * topic-alignment Preview acceptance.
 *
 ...
 *   - every course has the SAME delivery mode, so the delivery objectives
 *     cannot distinguish anything and that question stays suppressed;
...
const CONTENT: Record<string, string> = {
  E1: 'תכן הנדסי, וכן תהליכי ייצור ועיבוד שבבי.',
  E2: 'תכן הנדסי בלבד.',
...
function doc(courseId: string): SyllabusDocument {
  return {
    ...
    courseId,
    academicYear: YEAR,
    sourceUrl: `https://ims.tau.ac.il/Tal/Syllabus/Syllabus_L.aspx?course=topicpreview${courseId}&year=${YEAR}`,
    contentHash: `sha_topicpreview_${courseId}`,
    retrievedAt: '2026-08-15T00:00:00.000Z',
    labeledFields: { 'מספר קורס': [courseId], 'אופן ההוראה': ['שיעור'] },
    text: `אופן ההוראה שיעור תוכן הקורס ומטרתו ${CONTENT[courseId]} מטלות הקורס`,
  };
}

function main() {
  const root = join(process.cwd(), 'data', 'evidence_fixtures', 'topic_preview');
